...
function backup_tables($host,$user,$pass,$name,$nama_file,$tables = '*')
{
	...
	echo "<script>alert('Data Berhasil Di Backup di folder tmp');parent.location='index.php';</script>";
	$handle = fopen('../tmp/'.$nama_file,'w+');
	fwrite($handle,$return);
	fclose($handle);
}

$file="poliklinikTRSNW".date('d-m-Y').'.sql';
//panggil fungsi dengan memberi parameter untuk koneksi dan nama file untuk backup
$a = backup_tables("localhost","root","","poliklinik",$file);
